...
/* 商品类型选择函数 */
function gtypeInit(divId)
{
    $("#" + divId + " > select").get(0).onchange = gtypeChange; // select的onchange事件
    window.onerror = function(){return true;}; //屏蔽jquery报错
    $("#" + divId + " .edit_gcategory").click(gtypeEdit); // 编辑按钮的onclick事件
}
function gtypeChange()
{
    // 删除后面的select
    $(this).nextAll("select").remove();
	$("#type_id").val("");
    ...
    {
        var _self = this;
        var url = SITEURL + '/index.php?act=index&op=josn_type&callback=?';
        $.getJSON(url, {'type_id':this.value}, function(data){
            if (data)
            {
                if (data.length > 0)
                {
                    $("<select class='class-select'><option>-请选择-</option></select>").change(setSelect).insertAfter(_self);
                    var data  = data;
					...
					$("<span style='color:red;' >该目录无商品类型！</span>").insertAfter(_self);
				}
            }
        });
    }
}
//$array[$val['type_id']] = array('type_id'=>$val['type_id'],'type_name'=>htmlspecialchars($val['type_name']),'type_parent_id'=>$val['type_parent_id'],'type_sort'=>$val['type_sort']);
        ...
        $.getJSON(url, {'gc_id':this.value}, function(data){
            if (data)
            {
                if (data.length > 0)
                {
                    ...
                    for (i = 0; i < data.length; i++)
                    {
                        $(_self).next("input").append("<option data-explain='" + data[i].commis_rate + "' value='" + data[i].gc_id + "'>" + data[i].gc_name + "</option>");
                    }
                }
            }
        });
